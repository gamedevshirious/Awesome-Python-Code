...
<Screen> :
	...
	GridLayout :		
		...
		size_hint : 1 , .70
		Label :
			text : "Speaking Scientific Calculator"
			size_hint : .8 , .2
		Button :
			id : sound
			text : "Sound : On"
	...
	GridLayout :
		...
		Button :
			text : "Speak o/p"
			on_press : if deqn.text != "" and deqn.text != ")" : root.calculate(True)
		Button :
			text : "="
...
class Screen(BoxLayout) :
	...
	def calculate(self , speak_ans = False):
		try :
			...
			speak_ans = False if self.ids.sound.text == "Sound : Off" or speak_ans == False else True
			if speak_ans == True : tts.speak(str(Ans))
